...
void
http_folder_perms_req_free (HttpFolderPermReq *req)
{
    if (!req)
        return;
    g_free (req->token);
...
void
http_folder_perms_res_free (HttpFolderPermRes *res)
{
    GList *ptr;
...
out:
    ...
    if (ret < 0) {
        for (ptr = results; ptr; ptr = ptr->next)
            http_folder_perms_res_free ((HttpFolderPermRes *)ptr->data);
        g_list_free (results);
    } else
...
get_folder_perms_thread (void *vdata)
{
    ...
    req_content = compose_folder_perms_request (data->requests);
    if (!req_content)
        goto out;
